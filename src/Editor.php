...
class Editor
{
     ...
     * @return Editor
     */
    public function replaceSub(string $replacement, int $start, int $length = null)
    {
     ...
     * @return Editor
     */
    public function slugify(string $separator = '-', string $language = 'en')
    {
     ...
     * @throws \Exception
     */
    function split(int $splitLength = 1): array
    {
        if ($splitLength == 0) {
            throw new \Exception(
        ...
        for ($ii = 0; $ii < $length; $ii += $splitLength) {
            $result[] = $this->slice($ii, $splitLength);
        }
    ...
    /**
     * Unicode safely split a string into an array of words.
     *
    ...
    {
        $delimitersArray = static::create($delimiters)->split(1);
